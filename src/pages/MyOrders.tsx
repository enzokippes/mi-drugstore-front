...
import { Package, ArrowLeft, MapPin, Phone, Clock, Truck, Store as StoreIcon } from 'lucide-react';
  ...
  return (
    <div className="max-w-3xl mx-auto animate-fade-in">
      <div className="mb-8 flex items-center gap-3">
        <Link to="/" className="text-gray-500 hover:text-green-500 transition-colors p-2 hover:bg-gray-800 rounded-xl">
      ...
      {orders.length === 0 ? (
        <div className="bg-gray-900 border border-gray-800 rounded-xl p-16 text-center">
          ...
          <p className="text-white text-lg font-semibold mb-1">No tenés pedidos</p>
          <p className="text-gray-500 text-sm mb-6">Cuando hagas un pedido, va a aparecer acá.</p>
          <Link to="/" className="inline-block bg-green-600 text-white px-6 py-2.5 rounded-xl hover:bg-green-700 transition-colors font-medium text-sm">
            Ir a la tienda
          </Link>
        </div>
      ) : (
        <div className="space-y-4">
          {orders.map((order) => (
            <div key={order.id} className="bg-gray-900 border border-gray-800 rounded-xl overflow-hidden">
              <div className="bg-gray-800/50 px-5 py-3 flex items-center justify-between border-b border-gray-800">
              ...
              <div className="px-5 py-4">
                ...
                <div className="border-t border-gray-800 mt-4 pt-3 flex justify-between items-center">
                  ...
                  <span className="text-green-500 font-bold text-xl">${order.total.toLocaleString('es-AR')}</span>
                </div>
              </div>
            </div>
          ))}
